...
<body>
  <main>
      ...
      <section class="panel">
        ...
        <form id="ocr-form">
          ...
          <select id="engine-type" name="engine_type">
            ...
            <option value="agent">Agent OpenClaw/Codex</option>
            <option value="vps-local">VPS Local PaddleOCR</option>
            <option value="vps-agent">VPS Agent OpenClaw/Codex</option>
          </select>
    ...
    <section class="api-docs">
      ...
      <p>Gunakan service OCR sebagai API multipart upload. Web cukup mengirim file dokumen dan membaca JSON hasil ekstraksi.</p>
      <div class="api-warning">
        Jangan taruh API key langsung di frontend publik. Kalau halaman ini dinaikkan ke GitHub/Vercel, simpan API key di environment variable serverless/API route, lalu frontend memanggil route proxy tersebut.
      </div>
      <div class="api-grid">
...
curl -X POST "http://203.194.113.161/ocr/stnk" \
  ...
  -F "file=@STNK.pdf"</code></pre>
      ...
      <h3>Contoh JavaScript dari Frontend ke Backend Proxy</h3>
...
if (result.input_assessment?.can_auto_publish) {
  // lanjut proses pembelian
} else {
  // tampilkan review / minta upload ulang
}</code></pre>

      <h3>Contoh Vercel API Route Proxy</h3>
      <pre><code>// app/api/ocr/ktp/route.js
export async function POST(request) {
  const body = await request.formData();
  const response = await fetch("http://203.194.113.161/ocr/ktp", {
    method: "POST",
    headers: {
  ...
  "fields": {
    "nik": { "value": "3374114208810004", "status": "ok" },
    "nama": { "value": "DANI ANGGOROWATI", "status": "ok" }
  }
}</code></pre>
    </section>
  </main>
  ...
  <script>
    ...
    function endpointFor(documentType, engineType) {
      ...
      if (engineType === "agent") {
        if (documentType === "KTP") return "/ocr/agent/ktp";
        if (documentType === "STNK") return "/ocr/agent/stnk";
    ...
    function renderResult(data, roundtripMs) {
      ...
      if (isProcessingTimeout) {
        document.getElementById("initial-status").textContent = "OCR accurate masih berjalan di background";
        reviewBadge.className = "badge warn";
        reviewBadge.textContent = "Processing";
        assessmentEl.style.display = "block";
        assessmentEl.className = "assessment review";
        assessmentEl.textContent = "OCR fast melewati batas checkout. Hasil awal belum berisi field OCR; tunggu Background Enrichment selesai untuk hasil accurate.";
      }
      ...
      if (isProcessingTimeout) {
        ...
        row.innerHTML = '<td colspan="4" class="value">Hasil awal belum berisi field OCR. Lihat Background Enrichment setelah status selesai.</td>';
      ...
      renderEnrichmentPending(data.enrichment);
    }
    ...
    function renderEnrichmentPending(enrichment) {
      ...
      document.getElementById("enrichment-status").textContent = status === "queued" ? "Menunggu hasil enrichment" : status;
